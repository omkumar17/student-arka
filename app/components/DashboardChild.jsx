          ...
          {filteredBuses.length > 0 ? (
            filteredBuses.map((bus) => (
              <div key={bus.id} className="border-2 md:p-6 p-10 rounded shadow max-w-md ">
                <h2 className="text-lg font-semibold   "><span className="bg-green-800 p-2 text-white rounded-lg">Registration Number: {bus.reg_no}</span></h2><br/>
                {/* <p className=' '><strong className=' '>ID:</strong> {bus.id}</p> */}
                <p className='  capitalize text-lg'><strong className='text-blue-700 font-bold  mr-10 '>Bus No:</strong><span className="w-full">{bus.bus_no || 'Not set'}</span></p>
                ...
                <p className='  capitalize text-lg '><strong className='text-blue-700 font-bold  mr-10 '>Route:</strong> <span className="w-full text-wrap text-red-700">{bus.route || 'Not set'}</span></p>
